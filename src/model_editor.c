...
void UpdateModelEditor(ModelEditor *editor, SceneContext *context)
{
    ...
    else if (editor->currentTool == TOOL_MOVE && editor->selectedObjectIndex >= 0)
    {
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
        {
        ...
        if (editor->isDragging)
        {
            if (IsMouseButtonDown(MOUSE_LEFT_BUTTON))
            {
                ...
                Vector2 mousePos = GetMousePosition();
                Vector2 delta = Vector2Subtract(mousePos, (Vector2){editor->dragStart.x, editor->dragStart.y});

                // Apply movement to selected object
                // This is a simplified movement - in a real app, you'd project mouse movement to 3D space
                Vector3 movement = {
                    delta.x * 0.01f,
    ...
    else if (editor->currentTool == TOOL_ROTATE && editor->selectedObjectIndex >= 0)
    {
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
        {
            editor->isDragging = true;
    ...
    else if (editor->currentTool == TOOL_SCALE && editor->selectedObjectIndex >= 0)
    {
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
        {
            editor->isDragging = true;
...
void DrawModelEditor(ModelEditor *editor, SceneContext *context)
{
    ...
    // Tools
    DrawText(TextFormat("Tool: %s",
                        editor->currentTool == TOOL_SELECT ? "Select" : editor->currentTool == TOOL_MOVE ? "Move (G)"
    ...
    DrawText(TextFormat("Primitive: %s",
                        editor->currentPrimitive == SHAPE_CUBE ? "Cube" : editor->currentPrimitive == SHAPE_SPHERE ? "Sphere"
    ...
    // Controls
    DrawText("SPACE: Create | DEL: Delete | TAB: Switch Scene", 20, 110, 10, BLACK);
}
...
void SelectObject(ModelEditor *editor, Ray ray)
{
    ...
    for (int i = 0; i < editor->objectCount; i++)
    {
        ...
        // This is a simplified approach - in a real app, you'd transform the bounds properly
...
void CreatePrimitive(ModelEditor *editor, PrimitiveType type, Vector3 position)
{
    ...
    editor->objects = (EditorObject *)realloc(editor->objects, (editor->objectCount + 1) * sizeof(EditorObject));
    ...
    case SHAPE_PYRAMID:
        obj->model = LoadModelFromMesh(GenMeshCube(1.0f, 1.0f, 1.0f)); // Replace with a cube for now
        obj->color = YELLOW;
        break;
...
void DeleteSelectedObject(ModelEditor *editor)
{
    ...
    {
        editor->objects = (EditorObject *)realloc(editor->objects, editor->objectCount * sizeof(EditorObject));
    }
    else
    {
        free(editor->objects);
