    ...
    if (editor->currentTool == TOOL_SELECT) {
        ...
        if (editor->isDragging) {
            if (IsMouseButtonDown(MOUSE_LEFT_BUTTON)) {
                ...
                Vector2 mousePos = GetMousePosition();
                Vector2 delta =
                    Vector2Subtract(mousePos, (Vector2){editor->dragStart.x, editor->dragStart.y});

                // Apply movement to selected object
                // This is a simplified movement - in a real app, you'd project mouse movement to 3D
                // space
                Vector3 movement = {delta.x * 0.01f,
...
void DrawModelEditor(ModelEditor* editor, SceneContext* context)
{
    ...
    for (int i = 0; i < editor->objectCount; i++) {
        ...
        DrawModelEx(obj->model, obj->position, (Vector3){0.0f, 1.0f, 0.0f}, obj->rotation.y,
                    obj->scale, obj->selected ? RED : obj->color);
    ...
    // Tools
    DrawText(TextFormat("Tool: %s", editor->currentTool == TOOL_SELECT   ? "Select"
    ...
    // Primitive type
    DrawText(TextFormat("Primitive: %s", editor->currentPrimitive == SHAPE_CUBE       ? "Cube"
...
void CreatePrimitive(ModelEditor* editor, PrimitiveType type, Vector3 position)
{
    ...
    editor->objects =
        (EditorObject*)realloc(editor->objects, (editor->objectCount + 1) * sizeof(EditorObject));
    ...
    switch (type) {
        ...
        case SHAPE_PYRAMID:
            obj->model =
                LoadModelFromMesh(GenMeshCube(1.0f, 1.0f, 1.0f)); // Replace with a cube for now
            obj->color = YELLOW;
            break;
...
void DeleteSelectedObject(ModelEditor* editor)
{
    ...
    if (editor->objectCount > 0) {
        editor->objects =
            (EditorObject*)realloc(editor->objects, editor->objectCount * sizeof(EditorObject));
    } else {
        free(editor->objects);
        editor->objects = NULL;
    }
