    ...
    for (int i = 0; i < world->objectCount; i++)
    {
        ...
        if (obj->hasPhysics)
        {
            ...
            obj->velocity = Vector3Add(obj->velocity,
                                       Vector3Scale(obj->acceleration, deltaTime));

            // Update position
            obj->position = Vector3Add(obj->position,
                                       Vector3Scale(obj->velocity, deltaTime));
...
void UpdateFPSCamera(GameWorld *world)
{
    ...
    {
        // Move forward
        Vector3 forward = Vector3Normalize(Vector3Subtract(world->playerCamera.target,
                                                           world->playerCamera.position));
    ...
    {
        // Move backward
        Vector3 backward = Vector3Normalize(Vector3Subtract(world->playerCamera.position,
                                                            world->playerCamera.target));
    ...
    if (IsKeyDown(KEY_LEFT_ALT))
    {
        ...
        if (mouseDelta.x != 0)
        {
            Vector3 forward = Vector3Subtract(world->playerCamera.target, world->playerCamera.position);
...
void DrawGameWorld(GameWorld *world, SceneContext *context)
{
    ...
    // Physics status
    DrawText(TextFormat("Physics: %s (P to toggle)",
                        world->physicsEnabled ? "ON" : "OFF"),
             20, 50, 10, BLACK);

    // Camera mode
    DrawText(TextFormat("Camera: %s (F to toggle)",
