    ...
    switch (app->sceneContext.currentScene) {
        ...
        case SCENE_MODEL_EDITOR:
            ...
            if (IsKeyPressed(KEY_G))
                app->modelEditor.currentTool = TOOL_MOVE;
            if (IsKeyPressed(KEY_R))
                app->modelEditor.currentTool = TOOL_ROTATE;
            if (IsKeyPressed(KEY_S))
                app->modelEditor.currentTool = TOOL_SCALE;
            if (IsKeyPressed(KEY_TAB))
                app->modelEditor.currentTool = TOOL_SELECT;

            // Primitive selection
            if (IsKeyPressed(KEY_ONE))
                app->modelEditor.currentPrimitive = SHAPE_CUBE;
            if (IsKeyPressed(KEY_TWO))
...
void DrawApp(AppState* app)
{
    ...
    switch (app->sceneContext.currentScene) {
        ...
        case SCENE_EXPORT:
            ...
            DrawText(TextFormat("Filename: %s", app->exportSettings.filename), 20, 80, 20,
                     DARKGRAY);
            DrawText(
                TextFormat("Format: %s", app->exportSettings.useAsciiFormat ? "ASCII" : "Binary"),
                20, 110, 20, DARKGRAY);
            DrawText(TextFormat("Combine objects: %s",
                                app->exportSettings.combineObjects ? "Yes" : "No"),
                     20, 140, 20, DARKGRAY);
            DrawText(TextFormat("Scale to mm: %s",
                                app->exportSettings.scaleToMillimeters ? "Yes" : "No"),
                     20, 170, 20, DARKGRAY);
