...
void UpdateCurrentScene(SceneContext *context)
{
    ...
    case SCENE_GAME_WORLD:
        ...
        if (wheelMove != 0)
        {
            Vector3 direction = Vector3Normalize(Vector3Subtract(context->camera.target, context->camera.position));
            context->camera.position = Vector3Add(context->camera.position,
