...
void UpdateCurrentScene(SceneContext *context) {
  ...
  switch (context->currentScene) {
    ...
    case SCENE_GAME_WORLD:
      ...
      if (IsKeyDown(KEY_W)) context->camera.position.z -= 0.2f;
      if (IsKeyDown(KEY_S)) context->camera.position.z += 0.2f;
      if (IsKeyDown(KEY_A)) context->camera.position.x -= 0.2f;
      if (IsKeyDown(KEY_D)) context->camera.position.x += 0.2f;
      ...
      if (wheelMove != 0) {
        ...
        context->camera.position =
            Vector3Add(context->camera.position,
                       Vector3Scale(direction, wheelMove * 0.5f));
      }
