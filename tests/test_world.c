...
// Tests can focus on pure data manipulation
static void test_object_management() {
  GameWorld world;
  InitGameWorld(&world);

  assert(world.objectCount == 0);

  AddObject(&world, (WorldVector3){0, 0, 0}, (WorldVector3){1, 1, 1},
            (WorldColor){255, 255, 255, 255});
  assert(world.objectCount == 1);

  RemoveObject(&world, 0);
  assert(world.objectCount == 0);
